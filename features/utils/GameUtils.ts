 ...
 */
export const isOnGameBall = (gameLog: GameLog, matchRules: MatchRules): boolean => {
 ...
 */
export const gameWinner = (gameLog: GameLog, matchRules: MatchRules): number | undefined => {
    const pointsPerGame = matchRules.getPointsPerGame();
    const winningRequirement = matchRules.getWinningRequirement();
    ...
    switch(winningRequirement) {
        ...
        case WinningRequirement.WinByTwo: {
            ...
            if(pointDifference >= 2) {
                // Check if P1 is winner
                if(player1Points > player2Points && player1Points >= pointsPerGame) return playerIds.at(0)
                // Check if P2 is winner
                if(player1Points > player2Points && player1Points >= pointsPerGame) return playerIds.at(0)
            }
